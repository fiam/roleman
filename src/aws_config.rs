...
mod tests {
    ...
    fn sanitizes_profile_components() {
        assert_eq!(sanitize_component("Docker Cloud/Prod"), "Docker-Cloud-Prod");
    }
    ...
    fn ensures_config_profile() {
        ...
        ensure_profile_region("Docker-Cloud/ReadOnly", "us-east-1").unwrap();
        let config_path = roleman_aws_config_path().unwrap();
        let contents = fs::read_to_string(config_path).unwrap();
        assert!(contents.contains("[profile Docker-Cloud/ReadOnly]"));
